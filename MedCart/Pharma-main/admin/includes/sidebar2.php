...
// === Rider Login Check ===
if (!isset($_SESSION['rider_email'], $_SESSION['rider_role_id']) || $_SESSION['rider_role_id'] != 3) {
...
// === Functions ===
function update_rider_profile_pic($conn, $email, $filename) {
    $stmt = $conn->prepare("UPDATE users SET profile_pic = ? WHERE email = ? AND role_id = 3");
...
function get_rider_profile_pic($conn, $email) {
    $stmt = $conn->prepare("SELECT profile_pic FROM users WHERE email = ? AND role_id = 3 LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['select_existing'], $_POST['selected_pic'])) {
    ...
    if ($imagePath && strpos($imagePath, $imageDir) === 0 && file_exists($imagePath)) {
        if (update_rider_profile_pic($conn, $email, $selected)) {
            $_SESSION['rider_profile_pic'] = $selected;
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload_new']) && isset($_FILES['new_pic'])) {
    ...
    if (in_array($ext, $allowedExts)) {
        ...
        if (move_uploaded_file($fileTmp, $targetPath)) {
            if (update_rider_profile_pic($conn, $email, $newName)) {
                $_SESSION['rider_profile_pic'] = $newName;
            }
        }
    }
...
$dbPic = get_rider_profile_pic($conn, $email);
    ...
    <div class="position-sticky d-flex flex-column h-100 pt-3 justify-content-between">
        <div>
            ...
            <img src="../images/<?= htmlspecialchars($pic) ?>?v=<?= time() ?>" alt="Profile Picture" width="50" height="50"
                ...
                <br>
                <strong><?= htmlspecialchars($_SESSION['rider_fname'] ?? '') . ' ' . htmlspecialchars($_SESSION['rider_lname'] ?? '') ?></strong><br>
                <small><?= htmlspecialchars($email) ?></small>
            </div>

            <ul class="nav flex-column mt-3">
                <li class="nav-item"><a class="nav-link" href="rider-dashboard.php"><span data-feather="home"></span> Dashboard</a></li>
                <li class="nav-item"><a class="nav-link" href="rider-orders.php"><span data-feather="truck"></span> Assigned Orders</a></li>
                <li class="nav-item"><a class="nav-link" href="view_rider_reviews.php"><span data-feather="star"></span> Reviews</a></li>
            </ul>
        </div>
...
<nav class="navbar navbar-light bg-light d-md-none">
  <div class="container-fluid">
    ...
    <span class="navbar mb-0 h3">Rider Panel</span>
  </div>
</nav>
